...
def tweet_text(tweet):
    ...
    "*** YOUR CODE HERE ***"
    return getitem(tweet, 'text')

def tweet_time(tweet):
    """Return the datetime representing when a tweet was posted."""
    "*** YOUR CODE HERE ***"
    return getitem(tweet, 'time')

def tweet_location(tweet):
    """Return a position representing a tweet's location."""
    "*** YOUR CODE HERE ***"
    return make_position(getitem(tweet, 'latitude'), getitem(tweet,'longitude'))
...
def analyze_tweet_sentiment(tweet):
    ...
    "*** YOUR CODE HERE ***"
    x = tweet_words(tweet)
    total, words_with_sent = 0, 0
    for i in x:
        if has_sentiment(get_word_sentiment(i)) != False: 
            total += sentiment_value(get_word_sentiment(i))
            words_with_sent+=1
    if words_with_sent==0:
        average=make_sentiment(None)
    else:
        average=make_sentiment(total/words_with_sent)
    return average
...
def find_centroid(polygon):
    ...
    def area(polygon): 
        ...
        while k < len(polygon)-1:
            total += (latitude(polygon[k])*longitude(polygon[k+1])) - (latitude(polygon[k+1])*longitude(polygon[k]))    
            k += 1
        return (total/2)

    def lat_centroid(polygon):
        total = 0 ; k = 0 
        while k < len(polygon)-1:
            total += (latitude(polygon[k])+latitude(polygon[k+1]))*((latitude(polygon[k])*longitude(polygon[k+1]))- (latitude(polygon[k+1])*longitude(polygon[k])))    
            k += 1
        return total/(6*(area(polygon)))

    def long_centroid(polygon):
        total = 0 ; k = 0 
        while k < len(polygon)-1:
            total += (longitude(polygon[k])+longitude(polygon[k+1]))*((latitude(polygon[k])*longitude(polygon[k+1]))- (latitude(polygon[k+1])*longitude(polygon[k])))    
            k += 1
        return total/(6*(area(polygon)))

    if area(polygon)==0:
        return latitude(polygon[0]), longitude(polygon[0]), 0 
    return lat_centroid(polygon), long_centroid(polygon), abs(area(polygon))
...
def find_state_center(polygons):
    ...
    def avg_centroid_x(polygons):
        total, k=0, 0
        while k<len(polygons):
            total, k=total+((find_centroid(polygons[k])[0])*find_centroid(polygons[k])[2]), k+1
        return total
    def avg_centroid_y(polygons):
        total, k=0, 0
        while k<len(polygons):
            total, k=total+((find_centroid(polygons[k])[1])*find_centroid(polygons[k])[2]), k+1
        return total
    def avg_area(polygons):
        total, k=0, 0
        while k<len(polygons):
            total, k=find_centroid(polygons[k])[2]+total, k+1
        return total
    return make_position(avg_centroid_x(polygons)/avg_area(polygons), avg_centroid_y(polygons)/avg_area(polygons))
...
def average_sentiments(tweets_by_state):
    ...
    for i in tweets_by_state:
        if len(i)!=0:
            count, avg=0, 0
            for twt in tweets_by_state[i]:
                tot = analyze_tweet_sentiment(twt)
                if has_sentiment(tot):
                    avg=sentiment_value(tot)+avg
                    count+=1
            if avg!=0 and count!=0:
                averaged_state_sentiments[i]=avg/count
